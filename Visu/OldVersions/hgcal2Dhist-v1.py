...
def extractHistoFromEvent( forward, pid, he, x, y, z, histos, labels, labelIDs  ):
    """
    Input arguments 
      pid - particle ID
      he = hit enargy
    ...
    print pid
    ll = ( pidToName[p] for p in pid)
    k = pidToIdx[ pid[0] ]
    print "  OK  more", ", ".join(ll), part[ k ]
    part[ k ] += 1 
    ...
    histos.append( h )
    labels.append( pidToClass[ pid[0] ] )
    # labelIDs.append( pidToClassID[ pid[0] ] )
    labelIDs.append( np.array( [ pidToClassID[pid[0]] ], dtype=np.int32) )
...
def processEvent( tree, evID=0, eFilter=10.0, histos=None, labels=None, labelIDs=None, nPartInDetect=1 ):
#def processEvent( tree, evID, eFilter, histos, labels, labelIDs ):
  ...
  if (len(ef) != 0) and ( len(ef)<= nPartInDetect):
    extractHistoFromEvent( True, pidf, he, x, y, z, histos, labels, labelIDs  )
    nHistos +=2

  # Backward
  if (len(eb) != 0) and ( len(eb)<= nPartInDetect):
    extractHistoFromEvent( False, pidb, he, x, y, z, histos, labels, labelIDs  )
    nHistos +=2
...
def getHistos ( fname, startEventID=0, nHistos=20, fObjName="train.obj" ):
  ...
  while i < nHistos:
     n = processEvent( tree, evID=curentEvID, eFilter=pEnergyCut, histos=histos, labels=labels, labelIDs=labelIDs, nPartInDetect=50 )
     for k in range(n):
...
if __name__ == "__main__":
  ...
  getHistos ( fname, startEventID=0, nHistos=20, fObjName="train-ev.obj" )
