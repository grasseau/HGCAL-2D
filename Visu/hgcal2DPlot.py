...
def plotAnalyseEvent ( ev, histos, labels, rbbox = None, ellipse=None, sHits=None ):
  ...
  # Title
  id = str( ev[0].ID )
  partName    =  Config.pidToLatex [ ev[0].pid[0] ]
...
def plotAnalyseDataSet ( obj, i ):
  from matplotlib.colors import ListedColormap
  from matplotlib import colors

  ax = plt.subplot(1, 1,1)

  (images, bboxes_, labels_, labelIDs_, evIDs_, ellipses_, hEnergies_) = obj
  img = images[i]
  bboxes = bboxes_[i]
  labels = labels_[i]
  ...
  #
  imgplot = plt.imshow(img, label=labels, vmin=0.0, vmax=vmax, norm=norm)
  plt.title( "Compound ev. " +  str(i),  fontsize=16 )
  plt.suptitle( "Events : " + str(evIDs) )
